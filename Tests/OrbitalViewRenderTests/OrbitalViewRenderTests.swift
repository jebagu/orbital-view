...
final class OrbitalViewRenderTests: XCTestCase {
    ...
    func testSpeakerDrawInputsExposeInstancedCubePrismMeshAndMeterMaterial() throws {
        ...
        XCTAssertEqual(inputs.orientations[1], SIMD4<Float>(0, 1, 0, 1.75))
        XCTAssertEqual(inputs.materials.count, 3)
        XCTAssertEqual(inputs.materials[0].x, 0.25, accuracy: 0.0001)
        XCTAssertEqual(inputs.materials[0].y, 0.6, accuracy: 0.0001)
        XCTAssertEqual(inputs.materials[0].z, 0, accuracy: 0.0001)
        XCTAssertEqual(inputs.materials[1].x, 0.5, accuracy: 0.0001)
        XCTAssertEqual(inputs.materials[1].y, 0.8, accuracy: 0.0001)
        XCTAssertEqual(inputs.materials[1].z, 1, accuracy: 0.0001)
