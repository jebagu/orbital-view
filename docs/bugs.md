...
```text
2026-05-21 - Standalone native Orbital View VU Kit PNG export captured only the transparent SceneKit sphere viewport, and the rear-depth visual balance favored bright rear speakers over rear shell structure. Root cause: export was owned by the SceneKit view snapshot path, rear speaker material emission/alpha was still strong, rear shell edges were clipped by default hidden-line behavior, and struts were physically too thin. Fixed by moving export to a full application-window snapshot, increasing speaker-label and shell-strut sizing, keeping faint rear shell edges visible under fog, and adding camera-depth speaker alpha/emission attenuation.
