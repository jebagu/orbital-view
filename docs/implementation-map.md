...
`OrbitalViewportMockup` keeps the browser mockup as a loose behavior/control-inventory reference while using Orbisonic design language as the control-skin source of truth. It renders native 3D SceneKit geometry with fixed left/right rails, native SwiftUI/AppKit-backed controls, Fey 30 speaker coordinates, generated 3V geodesic count contract, Purple/Prism/Isometric defaults, fake meter stream, color schemes, camera controls, speaker shape controls, speaker size, fog density, speaker numbers, hidden lines, selection, spin, drag orbit, wheel/magnification zoom, inspector, speaker list, footer, and PNG export. The SwiftUI shell is mostly static, with no root animation timeline; the inspector/meter list owns a 10 fps refresh subview while the SceneKit coordinator owns fake meter and spin animation with a 30 fps active-motion cap and a 10 fps meter-only idle cadence. The SceneKit view draws on demand, disables implicit per-frame actions, builds shell geometry once, rebuilds speaker nodes only for shape/size changes, and separates camera, shell, speaker visibility, speaker material, and fog update keys. The SceneKit path orbits the camera around static content and uses SceneKit camera-space fog instead of rotating the model root or applying fog-driven material alpha fades. The app-bundle builder defaults to a release binary for local performance review. This is a native review app and does not replace the accepted production MetalKit renderer backend.
