...
Wave Relay source-lane telemetry is consumed through the local `OrbitalViewTelemetry` target rather than through a downstream app dependency. `Sources/OrbitalViewTelemetry/OrbitalViewTelemetryConsumer.swift` discovers providers with either `speakerMeters` or `sourceLaneMeters`, keeps speaker-meter automatic priority, attaches a reader for the selected slot, decodes `sourceLaneMeters` into meter keys `1...30`, and derives a local Wave Relay-compatible `-50...0 dBFS` display drive while preserving raw RMS/peak/clip facts. The SceneKit review surface in `Sources/OrbitalViewReview/OrbitalViewportMockup.swift` labels source-lane snapshots as `Source Lane VU` and lane cells instead of DVS channel output.
