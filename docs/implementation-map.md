...
`OrbitalViewportMockup` keeps the browser mockup as a loose behavior/control-inventory reference while using Orbisonic design language as the control-skin source of truth. It renders native 3D SceneKit geometry with fixed left/right rails, native SwiftUI/AppKit-backed controls, Fey 30 speaker coordinates, generated 3V geodesic count contract, Purple/Prism/Isometric defaults, fake meter stream, color schemes, camera controls, speaker shape controls, speaker size, fog density, speaker numbers, hidden lines, selection, spin, drag orbit, wheel/magnification zoom, inspector, speaker list, footer, full-window PNG export, and a Motion FPS 30/60 toggle. The SwiftUI shell is mostly static, with no root animation timeline; the inspector/meter list owns a 10 fps refresh subview while the SceneKit coordinator owns fake meter and spin animation with a selectable 30/60 fps active-motion cap defaulting to 60 fps and a 10 fps meter-only idle cadence. The SceneKit view draws on demand, disables implicit per-frame actions, builds shell geometry once with 1.5x thicker struts, rebuilds speaker nodes only for shape/size changes, and separates camera, shell, speaker visibility, speaker material, and fog update keys. The SceneKit path orbits the camera around static content, uses SceneKit camera-space fog, and adds camera-depth material attenuation so rear speakers are more subdued while faint rear shell structure remains visible under fog. The app-bundle builder defaults to a release binary for local performance review. This is a native review app and does not replace the accepted production MetalKit renderer backend.
...
The current wrapper provides `OrbitalView`, an `NSViewRepresentable` bridge, optional bottom VU settings tray, object snapshot/settings forwarding, optional input diagnostics display, optional host-provided visual-preset store actions, and coordinator tests. A future production settings pass should add a dedicated Graphical Performance versus CPU Load section for FPS, draw cadence, trail/glow, and quality tradeoffs. SwiftUI gestures, toolbar controls, hit testing, and inspector UI are deferred.
