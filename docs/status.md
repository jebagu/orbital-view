...
The confirmed review app keeps the left control rail focused on product identity, Camera, and View Detail. The visible rail now fills the full desktop window height, shows the plain `Orbital View` title near the top using the Wavefield Receiver player-title font treatment, and places Camera/View Detail directly underneath. The right panel now starts with a `Sound Metering Input` header above the single expandable `Input` tray, followed by `Speaker and Source Layout`, `Roll the dice on looks`, `Theme`, Speaker Appearance, Sphere Appearance, Ground Appearance, Meter Behavior, and Diagnostics. All collapsible right-panel trays start closed by default; operators expand only the tray they need. `Speaker and Source Layout` has title-only `Sonic Sphere Speakers` and `Source Speakers` trays for SpatGRIS receiver speaker setup import/save/defaults, source setup import/save/defaults, source project metadata import, and review-only `/spat/serv` OSC listening on a validated UDP port; the trays include the SPAT XML kicker copy for receiver and source layouts. The `Input` tray begins with the Orbisonic-style `Telemetry` / `Local Song` / `Impulse Test` selector and folds all input behavior into one surface: telemetry provider/status/track details with selectable advertiser buttons when multiple review advertisers exist, local song choose-file/transport controls, impulse pattern controls, and `Meter Source` status. Speaker Type lives in the right `Speaker Shape` tray. `Theme` now starts with the global `Color Palette` tray before `Saved Themes`; it uses the Orbisonic palette list to drive app skin, Sonic speaker pixels, source marker pixels, sphere palette, ground palette, and Cube VU ramp together. `Roll the dice on looks` is a global review-only dice action with a centered icon-only dice button that randomizes view/visual state, including Color Palette, Sphere Palette, Sphere Geometry, and Ground Appearance, while preserving Input state, saved/default theme metadata, selected speaker, and diagnostics. `Speaker Appearance` now contains `Speaker Shape`, `Label Font`, the dynamic `Cube Surface` / `Jet Surface` tray, and `Bloom Style`; the separate Sonic Sphere speaker palette, Source Speaker Palette, and live Speaker Pattern future tray are removed. Label Font groups fonts by Normie, Nerd, and Nostromo families and includes one review-only font size slider shared by all speaker labels. `Sphere Geometry` owns the default-off `Ribbed Speaker Sphere` overlay plus Rib Thickness, Vertical Ribs, and Horizontal Rings controls. `Sphere Palette` owns the sphere palette/saturation styling, puts `Same as Main Theme` first, resolves follow-main state at render time, and lets explicit local choices override after a global choice; palette selection lifts the default zero saturation so the selected hue is visible on both ribbed-sphere color lanes in SceneKit and canvas fallback. `Ground Appearance` also puts `Same as Main Theme` first for the ground palette. Choosing a new global `Color Palette` resets both local palettes to follow main without overwriting their stored concrete override values. The old imported/Fey shell and `Hide Sphere` control are removed from the live review workflow. The left `View Detail` rail owns the optional display-only `Ground Plane` toggle after `Speaker Labels` and `Hidden Lines`; the review-only `Ground Appearance` tray owns `Grid Visibility`, `Grid Spacing`, `Grid Thickness`, and ground palette controls for the 10 x 10 line grid below the sphere without changing speaker geometry, labels, Cube VU materials, source markers, ribbed sphere style/topology, or meter state. New saved themes mirror the legacy-compatible `sourceSpeakerRenderStyle` field to the global color palette, while older separate source palette fields decode safely and no longer appear as live UI controls. Object overlay, trails, glow trails, and bounds controls are hidden for the current review pass while the underlying object contracts remain in the package.
...
`Cube VU` speaker type now uses the VU Kit scalar center-bloom contract in the SceneKit surface with a retained 9x9 face-pixel material texture applied to the actual cube faces, selected Orbisonic theme colors, RMS-driven center bloom, peak/hot fill, clip flash, and a material-only cube outline strength control. The old review-app `Speaker Height` control is no longer visible; older saved values still decode but are normalized to the current flat cube/prism geometry path. `Pixel Jets` and `Cell Jets` are display speaker types beside Cube VU. Pixel Jets is the face-pixel jet style: it uses the axial/cross pixel texture branch on the jet faces, stays nearly dark with no real meter signal, uses the selected Color Palette / VU ramp, and caps density at `1...9` so density `1` becomes full-width axial bands. Cell Jets reuses the same outward radial jet geometry and `Jet Length` control but renders retained coarse five-face cells/voxels with four sleeve faces plus one outward end cap per cell, no generated pixel textures, no SceneKit shader modifiers, and no per-meter geometry rebuild. Cell Jets alone exposes an `Idle Opacity` display control, defaulting to 100%, that can hide silent cells without suppressing active meter cells. The final Cell Jets end-capped cell stays dark unless the effective display scalar reaches full-scale or the meter clips. Cube Outline now applies to Cube VU, Pixel Jets, and Cell Jets, and speaker labels are offset from the furthest visible speaker/meter extremity with the same small Cube VU-style gap. Both jet styles are music-reactive only in normal input modes: time advances alone do not create trail/head pulses, and Local Song meters use only the mono music RMS/peak sample. Legacy `jetsVU` and `solidJets` settings decode as Pixel Jets; new saves encode `pixelJets` or `cellJets`. `Jet Length` is clamped to `8...180` pixels and defaults to `48` pixels for older JSON. The Jets modes are display-only; they do not mutate the canonical scene speaker geometry, physical channels, or static renderer scene cache. Prism and Sphere keep the existing simpler material tint behavior while inheriting the selected viewport theme.
...
- Added the Jets VU speaker display type to Core settings, the Metal renderer, the production SwiftUI tuning tray, the SceneKit review surface, saved-theme JSON, headless benchmark configuration, and regression tests.
- Added the Solid Jets low-CPU speaker display type to Core settings, the Metal renderer, the production SwiftUI tuning tray, the SceneKit review surface, saved-theme JSON, and regression tests.
- Renamed the accepted VU-reactive stacked jet style to Pixel Jets, added the efficient retained-cell Cell Jets style, and capped Pixel Density at `1...9` across Core, Metal, SwiftUI, and SceneKit review paths.
- Added Cell Jets idle opacity, extended Cube Outline to Pixel Jets and Cell Jets, kept Cell Jets tip caps dark until full-scale/clip, and moved speaker labels outward from each visible speaker/meter extremity.
- Changed SceneKit Cell Jets cells to five-face retained voxels: four sleeve faces plus one outward end cap using a single material per cell.
...
```text
- Resolve the failed Ribbed Speaker Sphere fog-depth visual pass. Current rejected paths are documented in `docs/fog-depth-failed-attempts.md`; do not treat SceneKit shader modifier plumbing or batch-wide material fog tint/dim as accepted visual proof. The current review surface intentionally removes the uniform full-sphere fog blanket while keeping global SceneKit atmosphere and ribbed cutaway movement.
...
### Update: 2026-06-02 Orbital View 1.1 — Review-App Overhaul

Large review-only (`OrbitalViewReview`) pass; production `OrbitalViewSwiftUI` / `OrbitalViewRender` / `OrbitalViewCore` untouched. Tagged `1.1`.

- **Inputs:** removed Local Song; inputs are now Telemetry · System Audio · Demo Patterns. New `OrbitalViewportSystemAudioTap` Core Audio process tap (macOS 14.2+, graceful fallback below) drives the visuals from live system audio. System Audio has a pattern selector (All Mono · Ripple · Waves · Orbiting Comets); the excited patterns are gated by the audio (silent → dark, no idle self-animation). Legacy `localSong` saved-theme values decode to Telemetry.
- **Demo Patterns:** "Impulse Test" renamed to "Demo Patterns".
- **Speaker shapes:** Prism & Sphere hidden from the UI (kept in the type/render layer for decode compat).
- **Sphere geometry:** Off / Ribbed / Lamella selector. New Lamella style = horizontal hoop rings + a staggered 45° diamond strut lattice with a **Number of Sectors** control. New **Structure Opacity** control fades the overlay (brightness-scaled so it actually fades on the dark theme; struts stop writing depth below 100% so it reads see-through).
- **Other UI:** Bloom Style hidden unless Cube VU; label-font size cap raised to 200%; Input Calibration moved to the left "View Options" section; Performance tray hidden and frame rate locked to 60fps; pinch/scroll zoom max raised to 2.0×; Jet "Fill From Outside" toggle (reverses meter origin); footer "DomeLab panel" text removed; default Pixel Density 9 → 4.
- **Themes:** saved theme files now use the `.orbvu` extension (legacy `.json` still read); added Import / Export / Delete for saved themes; **themes no longer carry or restore input state** (changing theme never changes the current input). Set-Default now refreshes the list; Saved Themes buttons no longer truncate.
- **Source Speakers:** the Refresh button is now Clear (removes all source markers/setup/project).
- **Dice ("Roll the dice"):** now randomizes ribbed↔lamella style, lamella sectors, structure opacity, jet length, and "Fill From Outside"; no longer randomizes Input Calibration.
- **Tooling:** added an offscreen `--snapshot` mode to `OrbitalViewHeadlessBenchmark` (plus `--lamella` / `--structure-opacity`) for headless visual verification; launcher sets audio-capture usage descriptions for the tap permission prompt.
- Verification: `swift build` clean; `swift test --filter OrbitalViewSwiftUITests` 150/150 (the full suite still hangs on the pre-existing `OrbitalViewTelemetry` UDP tests, so it is run filtered). Live System Audio capture requires a TCC grant + audio playing and is verified by launching the app.
...
**1. Per-speaker Cube/Jet material skip**

- Added a per-speaker material dirty-check in the SceneKit review coordinator (`updateSpeakers`): when the meter-driven material pass runs, each speaker now re-applies its Cube VU / Pixel Jets / Cell Jets / prism material only if its bucketed `OrbitalViewportSpeakerMaterialVisualSignature` changed since it was last applied; unchanged speakers are skipped. This avoids redundant face-texture cache lookups/generation and SceneKit material writes during live metering, when only some channels move between meter display buckets.
- The per-speaker signature is bucketed finer (scale 10,000) than the face-texture cache key (scale 96), so "signature unchanged" guarantees an identical texture + uniforms — this is a behavior-preserving, zero-visual-change optimization. It is strictly a refinement of the existing global `OrbitalViewportSpeakerMaterialVisualSignatureKey` gate, which still controls whether the per-speaker pass runs at all.
- Added `OrbitalViewportSpeakerMaterialContextKey` (renderStyle, cubeVUSettings, selectedChannel, speakerShape, speakerLabelFont) to invalidate every speaker's last-applied signature when a global visual input changes (palette/settings/selection/shape), so theme/settings/selection changes still re-apply all speakers immediately. `meterSourceMode` is deliberately excluded from the context because the live telemetry snapshot it carries changes every frame; per-speaker meter changes are already captured by the per-speaker signature.
- The per-speaker applied-signature memory (`lastAppliedSpeakerMaterialSignatures`) is also cleared in `rebuildSpeakers`, alongside the existing `speakerOutlineMaterialKeys` reset and the `lastSpeaker*Key = nil` resets in `rebuildSpeakerLabels`. This is required because a geometry rebuild (jet-length / speaker-size / face-pixel / shape change) recreates speaker nodes with fresh untextured materials; without the reset an unchanged meter signature would skip re-texturing and leave the new geometry blank. (This was caught by the existing Pixel Jets / Cell Jets regression tests during verification and fixed before this entry.)
- Extracted the inline per-speaker material work into `applySpeakerMaterial(...)` with no behavior change so the dirty-check can gate it cleanly.
- Diagnostics: added `speakerMaterialPerSpeakerSkipCount` to the render instrumentation snapshot and surfaced per-frame Cube VU texture-churn counters (renderScene / frameMaterialSkip / perSpeakerSkip / cubeMatUpdate / texHit / texMiss / texGen / texEvict / texAssign) in the `OrbitalViewHeadlessBenchmark` output for ongoing measurement.
- Measurement (release headless benchmark, 1360x820, 300 measured frames): the all-active impulse stress case is the WORST case for this fix because nearly every speaker moves every meter bucket, so few skips fire and throughput is unchanged — idle ~53→~55 FPS with `perSpeakerSkip=153/9000`, active ~189→~193 FPS with `perSpeakerSkip=3` (within run-to-run noise, i.e. no regression). The real-world benefit is partial-activity metering: the new `testCubeVUPerSpeakerMaterialSkipAvoidsRedundantUnchangedSpeakerWork` proves that when 1 of 30 channels changes, exactly 1 speaker re-applies and 29 are skipped, eliminating 29/30 of the per-bucket material/texture work for those speakers.
- The same benchmark diagnostics surfaced a separate, larger pre-existing hotspot for follow-up: under the all-active stress case the `OrbitalViewportCubeVUSceneKitMaterial` face-texture cache (160-entry FIFO, key quantized at scale 96) thrashes at ~9% hit rate and rasterizes a new `NSImage` (lockFocus + per-cell NSBezierPath fills) ~27 times/frame. A cache-limit sweep showed the working set far exceeds 160 and even an effectively unbounded cache only reaches ~53% hits, so cache-sizing alone is limited.

**2. Render-style palette cache**

- `OrbitalViewportRenderStyle.palette` was a computed property that rebuilt the entire palette — allocating every `Color` — on every access. Because `OrbitalViewportTheme` is constructed fresh each frame and queried many times per speaker, this showed up as the hottest stack on the SceneKit render queue under profiling. The palette is a pure function of compile-time color constants, so it is now precomputed once per style into a static `paletteCache` keyed by `OrbitalViewportRenderStyle`, with the former body moved to a private `computedPalette` that seeds the cache. The cached value is identical to the recomputed one — behavior-preserving, zero-visual-change.
- For the same reason, `kimiPurplePalette` (the `.purple` base palette, also wrapped by `daftPunkBow`) changed from a recomputed `static var` to a `static let`, removing its per-access rebuild while keeping the identical value.

**3. Saved-theme profiling harness**

- Added `OrbitalViewportThemeProfile.swift` (`OrbitalViewReview`), a measurement-only harness that profiles the SceneKit review render path for a *real saved theme* (e.g. "Quantum Grid", "Prism Comet") under spin + impulse meters, the way an operator drives it. It decodes a saved-theme JSON via the same `OrbitalViewportSettingsExportPayload` the app's "load saved theme" uses, rebuilds a faithful `OrbitalViewportRenderConfiguration` (reusing the same slider→value transforms), then drives `coordinator.renderHeadlessFrameForBenchmark` frame by frame, reporting whole-frame throughput plus per-aspect render instrumentation deltas (ribbed sphere, speaker materials, cell/cube materials, texture generation, fog) so we can see which sub-systems do work each frame.
- Wired it into `OrbitalViewHeadlessBenchmark` via a new `--theme <path-to-theme.json>` flag (repeatable for multiple themes). Theme mode forces spin + impulse and prints the per-aspect summary; the existing `--mode idle|active|both` synthetic path is unchanged. This is the measurement tool for the texture-churn follow-ups noted above.

- Verification: `swift build` passed; the `OrbitalViewSwiftUITests` suite passed 140/140 with 0 failures, including the two new per-speaker-skip tests and the previously at-risk instrumentation tests for selected-channel / palette / meter-bucket / outline / active-spin plus the Pixel/Cell Jets retained-material tests. (An earlier full-suite run in a network-capable environment reported 234 tests / 0 failures.)
- Known pre-existing test failures (unrelated to this work): in this session the `OrbitalViewTelemetryConsumerTests` UDP tests fail (`testLiveProviderAttachesReadOnlyAndPublishesChannelMeters`, `testExtendedSpeakerMeterFieldsDecodeDisplayDriveAndDVSState`, `testExtendedSpeakerMeterFieldsUseExplicitVUValidRecordFlag`) followed by an `Index out of range` crash, because the consumer never receives its loopback UDP packets (stays `No Provider` / `Waiting`, reads sentinel `-1.0`). This is environmental: the failures are identical on clean `HEAD` with this change stashed, persist with the sandbox disabled, and touch no code in this change (the `OrbitalViewTelemetry` module is not modified here). These tests should be re-run in an environment with working UDP loopback to confirm they return to green. The full `swift test` was intentionally not re-run in this pass because it hangs on these telemetry tests.
- Protected paths: none touched. `Sources/OrbitalViewSwiftUI/` and `Sources/OrbitalViewRender/` were not modified; this change is confined to the review-only `OrbitalViewReview` SceneKit surface plus its benchmark and tests.
- Follow-ups (not done here, ranked): (1) make `makeFaceTextureImage` cheaper — replace `NSImage.lockFocus` + per-cell `NSBezierPath` fills with a reused `CGContext`/bitmap and hoist the `NSColor` colorspace conversions out of the inner loop (zero-visual reduction of per-generation cost; helps the worst case the per-speaker skip cannot); (2) rethink the face-texture cache (larger budget, or generate at lower meter quantization) given the working-set finding above; (3) optionally coarsen the scale-96 display/hot quantization to shrink the working set quadratically — small visual change, needs explicit sign-off, consistent with the "focus on non-graphics, preserve the look" direction.

### Update: 2026-06-02 Render Perf: Resolved-Color Dictionary CoW Fix (biggest single win)

- Profiling the real themes under spin+impulse (`--theme` + `sample` of a 40k-frame render) showed the single largest CPU cost in the whole render path was `_NativeDictionary.copy()` (106 self-samples + 58 dealloc), called from `OrbitalViewportTheme.resolvedVURampColor(for:level:)` via Cell Jets' per-cell color resolution.
- Root cause: a classic Swift nested-dictionary copy-on-write trap. `resolvedVURampColor` wrote its cache as `var b = resolvedVURampColorBuckets[style] ?? [:]; b[bucket] = color; resolvedVURampColorBuckets[style] = b`. The `var` copy holds a second reference to the inner dictionary while it is mutated, so every insert deep-copies the entire inner dictionary. Cell Jets resolves a color per cell per speaker, the inner dictionary grows toward the 16,384-bucket scale, and the per-frame copies dominated CPU.
- Fix: insert through the defaulted-subscript in-place `_modify` accessor — `resolvedVURampColorBuckets[style, default: [:]][bucket] = color` — which mutates with no extra reference, so no copy occurs. Applied the same fix to `resolvedThemeColor` (small/bounded, kept consistent). Values are unchanged — purely removes the copy. No other nested-dict write in the file has this pattern (single-level caches like `faceTextureCache` are fine).
- Measurement (release `--theme`, CPU scene-mutation ms/frame, 3 runs): Quantum Grid 0.64 → ~0.345 ms (**−46%**); Prism Comet 1.22 → ~1.20 ms (≈flat — its bottleneck is the fog NSColor blend, not the cache write). Cumulative across all render-perf work: **Quantum Grid 0.89 → 0.345 ms = 2.6× faster**; Prism Comet 1.70 → 1.20 ms = 1.4×.
- Verification: `swift build` clean; full `swift test` 244/0 (suite grew with another session's additions). Existing color/material tests pass, confirming identical colors. Protected paths untouched.
- Next bottleneck (Prism Comet, fog-on): `foggedNSColor`→`blendDeviceRGB` plus Cell Jets' per-cell `mix(...)` allocate ~4–5 `NSColor` objects per cell per frame (NSColor object machinery = 705 self-samples). Getting Prism Comet to 2× means doing the Cell Jets color math on a lightweight RGBA-Double value type and constructing `NSColor`/`CGColor` only once per cell at `apply` — zero-visual but invasive surgery in Cell Jets' hot loop (which another session is actively editing).

### Update: 2026-06-02 Render Perf: Signature Dedup + Fog-Color Fast Path

Two more behavior-preserving (zero-visual-change) CPU reductions on the per-frame speaker path, confined to `OrbitalViewReview` + tests. Driven by faithful saved-theme profiling (`--theme`) of Quantum Grid and Prism Comet under spin + impulse, plus a live `sample` of the running app.

**1. Reuse per-speaker material signatures (eliminate duplicate computation)**

- `OrbitalViewportSpeakerMaterialVisualSignatureKey` already builds one `OrbitalViewportSpeakerMaterialVisualSignature` per speaker every frame (the key is constructed unconditionally to gate the speaker-material pass). `updateSpeakers` was then constructing the *same* 30 signatures again. Added `signaturesByChannel()` on the key and threaded the precomputed map into `updateSpeakers(... materialSignatures:)`, which now reuses them and only falls back to recomputing if a caller passes none. Removes 30 duplicate signature constructions per frame (each does VU-scalar/fog/alpha/emission math). Identical values — pure dedup.

**2. Fog-color cache + resolved-color fast blend**

- Profiling (headless + live `sample`) showed `foggedNSColor` and `OrbitalViewportTheme.fog` hot on the fogged-meter path: `foggedNSColor` bridged `Color`→`NSColor` for the fog color and ran two `usingColorSpace(.deviceRGB)` conversions inside `blend` on every call (per speaker per frame). This is why fog-on themes (Prism Comet, fog=10) cost ~2× the CPU scene-mutation of fog-off themes (Quantum Grid).
- Added `OrbitalViewportTheme.fogNSColor` (cached device-RGB fog color per style via the existing `resolvedThemeColor` machinery, new `.fog` role) and `OrbitalViewportColorTools.blendResolved`, a fast path that skips both colorspace conversions when both inputs are already `deviceRGB` (verified via `cgColor.colorSpace?.model == .rgb` + `colorSpace == .deviceRGB`, which is exception-safe) and otherwise falls back to `blend`. `foggedNSColor` now early-outs at zero fog and uses the cached fog + `blendResolved` with the already-resolved meter colors.
- Correctness: new `testBlendResolvedMatchesBlendForFoggedMeterColors` proves `blendResolved` is byte-identical to `blend` across all render styles, six heat levels, and six fog amounts (including the 0 and ~0.0005 edges).

- Measurement (release headless `--theme`, 1360x820, 300 frames, CPU scene-mutation ms/frame; GPU unchanged by design): Quantum Grid 0.72→~0.64 ms, Prism Comet 1.37→~1.22 ms (~11% each, stable across 3 runs). Cumulative with the prior palette cache: Quantum Grid 0.89→0.64 ms (−28%), Prism Comet 1.70→1.22 ms (−28%). The all-active synthetic benchmark is unaffected (it has no fog and few skips).
- Verification: `swift build` passed; focused tests pass (`BlendResolvedMatchesBlend`, both `PerSpeakerMaterialSkip`, `CubeVUResolvedMaterialColors`, meter-bucket/outline). Full `swift test` reported 239/0 in this session (suite grew from 234 with another session's additions). Release build required retries because another active session was editing `OrbitalViewportMockup.swift` concurrently ("modified during build"); the measured binary was built from a clean compile of the current source.
- Protected paths: none. `OrbitalViewSwiftUI/` and `OrbitalViewRender/` untouched. Shared-branch note: another session is concurrently editing `OrbitalViewportMockup.swift` (added `OrbitalViewportSystemAudioTap`); these edits were made surgically against re-read current content and left that work intact.
- Remaining follow-ups (unchanged, ranked): (1) cheaper `makeFaceTextureImage` (reused `CGContext` bitmap) for the Cube VU texture-churn worst case; (2) rethink the face-texture cache budget/quantization; (3) optional fog/quantization coarsening to let the per-speaker skip fire under spin (small visual change — needs sign-off; this is the true root of Prism Comet's residual cost).
...
- Changed the SceneKit Cell Jets geometry from open axial bands plus separate cap materials to one retained geometry element per cell.
- Each Cell Jets cell now contributes five colored faces: four sleeve sides plus one outward end cap, all driven by the same retained material/color for that cell; the inward/back face remains omitted.
- Removed separate SceneKit Cell Jets base/tip cap material dependency while preserving retained materials, no generated textures, no shader modifiers, no per-meter geometry rebuild, Idle Opacity behavior, Cube Outline, and label offset behavior.
- Preserved the final end-cap rule by muting the final cell until full-scale display (`>= 0.995`) or clip.
- Verification: `git diff --check`, `swift test --filter CellJets` (8 tests), `swift build`, full `swift test` (225 tests), `zsh -n Open\ Orbital\ View.command`, and the parent latest launcher all pass; `OrbitalViewViewer` is running from this checkout as pid `69814`.

### Update: 2026-05-31 Cell Jets And Jet Label Polish

- Added `cellJetsIdleOpacity` to `SpeakerMeterVisualSettings`, defaulting to `1.0`, validating `0...1`, and decoding older payloads as fully visible.
- Exposed `Idle Opacity` only for Cell Jets in the production SwiftUI tuning tray and SceneKit review `Jet Surface` tray; dice randomizers leave it at the visible default.
- Applied Cell Jets idle opacity to silent idle body/cap material state while keeping active meter cells visible from real signal.
- Extended Cube Outline child nodes and material updates to Pixel Jets and Cell Jets, using each jet block's current geometry dimensions.
- Kept the Cell Jets outward tip cap idle/dark unless the effective display scalar is full-scale (`>= 0.995`) or the channel clips, in both SceneKit and Metal paths.
- Repositioned SceneKit and canvas fallback speaker labels from the furthest visible speaker/meter extremity plus the small Cube VU-style gap, covering Prism, Sphere, Cube VU, Pixel Jets, and Cell Jets.
- Verification: `git diff --check` passed; `swift build` passed; focused `swift test --filter CellJets` passed with 8 tests and 0 failures; focused `swift test --filter Jet` passed with 17 tests and 0 failures; full `swift test` passed with 225 tests and 0 failures; `zsh -n Open\ Orbital\ View.command` passed; `/Users/jeremyguillory/Documents/vibecode projects/Open Orbital View Kit Latest.command` rebuilt/re-signed/opened the review app; `pgrep -fl /Contents/MacOS/OrbitalViewViewer` confirmed PID `66861` running from `/Users/jeremyguillory/Documents/vibecode projects/Orbital View Turbo/.../OrbitalViewViewer`.

### Update: 2026-05-31 Pixel Jets / Cell Jets Split

- Replaced the visible jet-style names with `Pixel Jets` and `Cell Jets` beside `Cube VU`; legacy `jetsVU` and `solidJets` JSON now decodes to Pixel Jets while new saves encode the new case names.
- Restored Pixel Jets to the face-pixel jet look, with axial/cross pixel textures on the long jet faces, dark no-provider/silent state, palette-ramp color authority, and no autonomous clock-driven pulse.
- Added Cell Jets as the separate low-CPU retained-cell style using the same outward radial geometry and `Jet Length`, with full-width stacked cell materials, no generated textures, no shader modifiers, and no per-meter geometry rebuild.
- Capped every Pixel Density path at `1...9`; density `1` now resolves to full-width jet bands/cells instead of high-density square pixels.
- Updated focused Core, Metal, SceneKit review, and SwiftUI tests for the new names, density cap, legacy migration, retained material branches, and palette/silence behavior.
- Verification: `git diff --check` passed; focused `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test --filter Jet` passed with 11 tests and 0 failures; `zsh -n Open\ Orbital\ View.command` passed; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift build` passed; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test` passed with 219 tests and 0 failures; `/Users/jeremyguillory/Documents/vibecode projects/Open Orbital View Kit Latest.command` rebuilt/re-signed/opened the review app; `pgrep -fl /Contents/MacOS/OrbitalViewViewer` confirmed PID `56877` running from `/Users/jeremyguillory/Documents/vibecode projects/Orbital View Turbo/.../OrbitalViewViewer`.

### Update: 2026-05-31 Pixel Jets Restoration And Music-Only Jet Motion

- Fixed the jet split regression where Pixel Jets and Cell Jets both rendered as the retained full-width cell style. Pixel Jets now uses the axial face-pixel texture path again, while Cell Jets remains the efficient retained-cell path.
- Removed autonomous time-phase trail/head motion from the active Metal jet branches and retained Cell Jets SceneKit material updates. Jet brightness, fill, leading edge, hot accents, and palette heat now come from VU meter scalars only.
- Changed Local Song speaker meters to return only the mono local-audio RMS/peak sample; selected local audio render modes no longer add synthetic impulse/excitement patterns. `Impulse Test` remains the intentional synthetic preview source.
- Removed the visible Local Song render-type controls from the review Input tray; legacy render-mode JSON still decodes for older themes but no longer drives visible speaker patterns.
- Updated focused Metal and SceneKit regressions to prove Pixel Jets uses face pixel textures, Cell Jets uses retained full-width cell materials, silent Pixel Jets stays dark and stable across time ticks, palette changes still affect Pixel Jets, and Cell Jets does not change when only time advances.
- Verification: `git diff --check` passed; focused `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test --filter Jet` passed with 11 tests and 0 failures; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift build` passed; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test` passed with 219 tests and 0 failures; `zsh -n Open\ Orbital\ View.command` passed; `/Users/jeremyguillory/Documents/vibecode projects/Open Orbital View Kit Latest.command` rebuilt/re-signed/opened the review app; `pgrep -fl /Contents/MacOS/OrbitalViewViewer` confirmed PID `61406` running from `/Users/jeremyguillory/Documents/vibecode projects/Orbital View Turbo/.../OrbitalViewViewer`.

### Update: 2026-05-31 Solid Jets VU-Reactive Palette Gate

- Changed Solid Jets from autonomous time-only pulsing to VU-gated motion: telemetry no-provider, local silence, and all-zero meter frames now render as a faint dark silhouette.
- Kept the stacked axial block look while driving active side bands, tip/base caps, trail motion, and clip accents from the selected speaker/VU palette ramp rather than fixed pink or standalone material colors.
- Updated Metal Solid Jets to use the existing color-ramp buffer and silence gate while preserving outward display-only geometry, channel identity, and static scene draw inputs.
- Added regression coverage for silent SceneKit Solid Jets darkness/no rebuilds, SceneKit palette-ramp response, Metal silent-vs-active intensity, Metal palette-ramp response, and shader signal gating.
- Verification: `git diff --check` passed; `zsh -n Open\ Orbital\ View.command` passed; focused `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test --filter SolidJets` passed with 5 tests and 0 failures; focused `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test --filter JetsVU` passed with 6 tests and 0 failures; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift build` passed; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test` passed with 219 tests and 0 failures; `/Users/jeremyguillory/Documents/vibecode projects/Open Orbital View Kit Latest.command` rebuilt/re-signed/opened the review app; `pgrep -fl /Contents/MacOS/OrbitalViewViewer` confirmed PID `50424` running from `/Users/jeremyguillory/Documents/vibecode projects/Orbital View Turbo/.../OrbitalViewViewer`.

### Update: 2026-05-31 Solid Jets Speaker Type

- Added `Solid Jets` as a third production speaker meter display type beside `Cube VU` and `Jets VU`; older settings payloads still default to Cube VU and `48` px jet length.
- Updated Metal to reuse outward radial jet geometry with a time-phased outward gradient branch, mode-separated from the Jets VU axial/cross pixel-cell shader path and independent of face-pixel density for Solid Jets length.
- Added SceneKit review support for `Solid Jets` in `Speaker Shape`, reusing `Jet Length` while assigning retained axial band materials plus simple base/tip caps and no SceneKit shader modifier or Jets VU axial texture generation.
- Added regression coverage for Core enum/display name, Metal Solid Jets geometry, shader mode, and moving phase, SwiftUI coordinator forwarding, settings JSON round-trip, and SceneKit retained animated band materials/no texture-cache use.
- Verification: `git diff --check` passed; `zsh -n Open\ Orbital\ View.command` passed; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift build` passed; focused `swift test --filter SolidJets` passed with 2 tests and 0 failures after the shader-free band-material fix; focused `swift test --filter JetsVU` passed with 6 tests and 0 failures; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test` passed with 216 tests and 0 failures; `/Users/jeremyguillory/Documents/vibecode projects/Open Orbital View Kit Latest.command` rebuilt/re-signed/opened the review app; `pgrep -fl /Contents/MacOS/OrbitalViewViewer` confirmed PID `46665` running from `/Users/jeremyguillory/Documents/vibecode projects/Orbital View Turbo/.../OrbitalViewViewer`.

### Update: 2026-05-31 Cube/Jet Surface Pixel Density Controls

- Renamed the surface face-count UI from `Face Pixels` to `Pixel Density` and changed it from a stepper to an integer slider at the top of the surface tray.
- The review surface tray now reads `Jet Surface` with `Randomize Jet Surface` when Jets VU is selected, and otherwise remains `Cube Surface` / `Randomize Cube Surface`.
- Production SwiftUI tuning now exposes `Pixel Density` and `Pixel Density Cost` sliders while preserving the existing `facePixels` settings field and renderer behavior.
- Added UI contract coverage for the renamed control list, dynamic Jet Surface title/randomizer label, and absence of the old `Face Pixels` label from the surface tray contracts.
- Verification: focused `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test --filter OrbitalViewSwiftUITests` passed with 130 tests and 0 failures; `git diff --check` passed; `zsh -n Open\ Orbital\ View.command` passed; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift build` passed; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test` passed with 214 tests and 0 failures; `/Users/jeremyguillory/Documents/vibecode projects/Open Orbital View Kit Latest.command` rebuilt/re-signed/opened the review app; `pgrep -fl /Contents/MacOS/OrbitalViewViewer` confirmed PID `19473` running from `/Users/jeremyguillory/Documents/vibecode projects/Orbital View Turbo/.../OrbitalViewViewer`.

### Update: 2026-05-31 Jets VU Square Pixel Layers

- Updated Jets VU so pixel density defines the cross-face pixel count while `Jet Length` resolves to an integer number of full square axial pixel layers.
- SceneKit Jets VU textures are now rectangular axial-by-cross pixel grids whose aspect ratio matches the snapped jet geometry, so longer jets add more square layers instead of stretching the same cells.
- Metal Jets VU now passes separate axial and cross pixel counts, snaps display-only jet depth to the same layer ratio, and quantizes shader cells independently on each axis.
- Added regression coverage for square layer metrics, texture-to-geometry aspect matching, and static draw input preservation while changing Jets VU length.
- Verification: focused `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test --filter JetsVU` passed with 6 tests and 0 failures; `git diff --check` passed; `zsh -n Open\ Orbital\ View.command` passed; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift build` passed; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test` passed with 214 tests and 0 failures; `/Users/jeremyguillory/Documents/vibecode projects/Open Orbital View Kit Latest.command` rebuilt/re-signed/opened the review app; `pgrep -fl /Contents/MacOS/OrbitalViewViewer` confirmed PID `18661` running from `/Users/jeremyguillory/Documents/vibecode projects/Orbital View Turbo/.../OrbitalViewViewer`.

### Update: 2026-05-31 Jets VU Pixel-Meter Rendering Fix

- Fixed Jets VU so it no longer falls through to the simple prism tint path. The SceneKit review app now uses a Jets-specific retained axial pixel texture on the long jet faces, darker base/tip cap materials, nearest-neighbor filtering, and Cube VU meter settings for face pixels, pixel fill, checker behavior, idle tint, hot fill, clip, and theme colors.
- Kept the accepted Jets VU geometry behavior: speaker count, outward radial jet shape, `Jet Length`, `Speaker Size`, Hidden Lines, channel order, saved settings, and display-only geometry/cache boundaries remain intact.
- Updated the Metal Jets VU shader branch from a smooth axial ramp to quantized axial/cross pixel cells that fill from the speaker sphere outward while continuing to use the selected `SpeakerMeterColorScheme` ramp.
- Added regression coverage for Jets VU material names, nearest-filter side textures, base-to-tip texture fill, clip tip cap behavior, Cube VU retained texture preservation, and Metal shader pixel quantization.
- Verification: focused Jets VU SceneKit and Metal regression tests passed; `git diff --check` passed; `zsh -n Open\ Orbital\ View.command` passed; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift build` passed; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test` passed with 213 tests and 0 failures; `/Users/jeremyguillory/Documents/vibecode projects/Open Orbital View Kit Latest.command` rebuilt/re-signed/opened the review app; `pgrep -fl /Contents/MacOS/OrbitalViewViewer` confirmed PID `16283` running from `/Users/jeremyguillory/Documents/vibecode projects/Orbital View Turbo/.../OrbitalViewViewer`.

### Update: 2026-05-31 Jets VU Speaker Type

- Added `SpeakerMeterSpeakerType` with `Cube VU` and `Jets VU`, plus `SpeakerMeterVisualSettings.speakerType` and `jetLengthPixels`. Old JSON defaults to Cube VU and `48` px, while validation rejects non-finite or out-of-range jet lengths outside `8...180`.
- Updated the Metal renderer to interpret Jets VU as display-only outward radial prism geometry from each speaker anchor, using pixel-quantized meter material payload and selected color ramp without changing the scene speaker shape, channel order, or static scene cache.
- Added production SwiftUI controls for `Speaker Type` and a Jets-only `Jet Length` slider; the existing `SpeakerMeterVisualStyle.cubeScalarCenterBloom` name and Cube VU behavior remain intact.
- Added SceneKit review support for `Jets VU` in the `Speaker Shape` tray, saved-theme JSON encode/decode, startup/default settings, canvas fallback sizing, and headless benchmark configuration.
- Added regression coverage for Core defaults/legacy decode/range validation, Metal offscreen Jets VU shape/color/meter behavior, SwiftUI coordinator forwarding, review settings payloads, and SceneKit Jets VU outward geometry sizing.
- Verification: `git diff --check` passed; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift build` passed; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test` passed with 211 tests and 0 failures; `/Users/jeremyguillory/Documents/vibecode projects/Open Orbital View Kit Latest.command` rebuilt/re-signed/opened the review app; `pgrep -fl /Contents/MacOS/OrbitalViewViewer` confirmed PID `13066` running from `/Users/jeremyguillory/Documents/vibecode projects/Orbital View Turbo/.../OrbitalViewViewer`.

### Update: 2026-05-31 Hidden Lines Ground Plane Isolation

- Fixed the native SceneKit review viewport so `Hidden Lines` no longer hides or clips the optional `Ground Plane` grid. Ground grid line nodes now render before the invisible ribbed-sphere cutaway depth plane, so ground visibility remains controlled by the `Ground Plane` toggle plus Ground Appearance settings.
- Preserved the existing ribbed-sphere hidden-line behavior: with `Hidden Lines` off, the cutaway plane still hides rear ribbed geometry; with `Hidden Lines` on, the cutaway plane is hidden.
- Added regression coverage for the SceneKit ordering case with Ground Plane on, Ribbed Speaker Sphere on, and Hidden Lines off.
- Verification: focused `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test --filter OrbitalViewSwiftUITests/testCorrectViewerHiddenLineCutawayDoesNotRenderBeforeGroundPlane` passed; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift build` passed; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test` passed with 208 tests and 0 failures; `/Users/jeremyguillory/Documents/vibecode projects/Open Orbital View Kit Latest.command` rebuilt/re-signed/opened the review app; `pgrep -fl /Contents/MacOS/OrbitalViewViewer` confirmed PID `9598` running from `/Users/jeremyguillory/Documents/vibecode projects/Orbital View Turbo/.../OrbitalViewViewer`.

### Update: 2026-05-31 Review Surface Palette, Grid, And Sphere Polish

- Added first-position `Same as Main Theme` choices to `Sphere Palette` and `Ground Appearance`. Follow-main is stored explicitly, resolved at render time to the global `Color Palette`, exported with compatibility resolved palette fields, and inferred for older JSON when the local palette matched the main palette.
- Added `Grid Thickness` under `Ground Appearance` with a `100%` default, `50%...250%` range, and `5%` UI step. SceneKit grid cylinder radii, canvas fallback line widths, render configuration, grid update keys, randomizer output, settings export/decode, and control inventory now include the thickness value.
- Removed the ribbed sphere's uniform batch-wide material fog tint/dim pass and kept fog changes on global SceneKit atmosphere plus ribbed cutaway-plane movement. The canvas fallback now draws the full-scene fog veil before the ribbed sphere so the sphere is not covered by a full-surface overlay.
- Brightened the two welded ribbed SceneKit material batches with a modest diffuse/emission lift and subtle physically based luster while preserving opaque depth reads/writes, shader-free materials, hidden-line cutaway behavior, and two-node batching.
- Verification: `git diff --check` passed; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test --filter OrbitalViewSwiftUITests` passed with 127 tests and 0 failures; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift build` passed; `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test` passed with 207 tests and 0 failures; `/Users/jeremyguillory/Documents/vibecode projects/Open Orbital View Kit Latest.command` rebuilt/re-signed/opened the review app; `pgrep -fl OrbitalViewViewer` confirmed PID `2451` running from `/Users/jeremyguillory/Documents/vibecode projects/Orbital View Turbo/.../OrbitalViewViewer`.

### Update: 2026-05-31 UI Palette Cleanup

- Renamed the left `View Detail` toggle from `Speaker Numbers` to `Speaker Labels` without changing the left rail width or control text sizing.
- Added the global `Theme` > `Color Palette` tray before `Saved Themes`; it drives app skin, Sonic speaker pixels, source marker pixels, sphere palette, ground palette, and Cube VU ramp together.
- Removed the live `Sonic Sphere Speaker Palette`, `Source Speaker Palette`, and `Speaker Pattern` future trays. `Speaker Appearance` now contains only `Speaker Shape`, `Label Font`, `Cube Surface`, and `Bloom Style`.
- Renamed the visible ribbed-sphere styling tray to `Sphere Palette` with `Sphere Palette` and `Sphere Saturation` controls. Local sphere and ground palette choices can still override after a global palette selection; choosing a new global palette resets those local palettes to match.
- Kept legacy saved-theme `sourceSpeakerRenderStyle` decoding for compatibility while new saves mirror it to the global `renderStyle`.
- Updated the project launcher to use `open -n` after refresh so the parent launcher can reliably relaunch the current review app after rebuilding and re-signing.
- Verification: `git diff --check` passed; `swift build` passed; `swift test --filter OrbitalViewSwiftUITests` passed with 126 tests and 0 failures; `swift test` passed with 206 tests and 0 failures; `zsh -n "Open Orbital View.command"` passed; `/Users/jeremyguillory/Documents/vibecode projects/Open Orbital View Kit Latest.command` rebuilt/re-signed/opened the review app; `pgrep -fl "/Contents/MacOS/OrbitalViewViewer"` confirmed the visible app running from this checkout.

### Update: 2026-05-31 Ribbed Sphere Live Magenta Shader Fallback Fix

- Diagnosed the live Ribbed Speaker Sphere hot pink/magenta output as a SceneKit shader fallback from the failed per-fragment ribbed fog shader modifier path, not stale SwiftUI state or coordinator propagation.
- Removed and cleared ribbed-sphere shader modifiers from the live SceneKit material path. The two batched ribbed mesh nodes, retained material diffuse/emission color authority, hidden-line cutaway plane, and independent Geodesic Appearance palette/saturation controls remain in place.
- Confirmed with `ORB_DEBUG_FORCE_RIBBED_GREEN=1` that the live node path reaches both vertical and horizontal rib batches, then linked normal Sonic Sphere palette clicks into the geodesic palette while the geodesic controls are not explicitly detached. Palette selection now lifts the exported default `0%` geodesic saturation so the chosen hue is visibly applied instead of remaining grayscale/purple-looking.
- Fixed the follow-up lane split where vertical ribs followed the selected theme but horizontal rings still used the theme's secondary accent. Both SceneKit rib batches and the canvas fallback now use the selected Geodesic/Sonic hue; the horizontal lane remains visually separated by alpha/brightness rather than by a different palette role.
- Added DEBUG-only live probes for the next visual issue: force-visible ribbed sphere, saved-theme suppression, random-animation suppression, render trace logging, force-green material override, continuous rendering, and an accessible ribbed debug overlay with build/style/saturation/apply/material-name fields.
- Added regression coverage that samples rendered SceneKit snapshot pixels and rejects Rack Blue output that is still magenta-dominant, plus material-name and debug-overlay checks for the live configuration.
- Verification: `git diff --check` passed; `swift build` passed; `swift test --filter OrbitalViewSwiftUITests/testCorrectViewerPaletteSelectionMakesDefaultGeodesicHueVisible` passed; `swift test --filter OrbitalViewSwiftUITests/testCorrectViewerSelectedGeodesicHueAppliesToBothRibbedLanes` passed; `swift test --filter OrbitalViewSwiftUITests/testCorrectViewerRackBlueChangesRenderedRibbedSpherePixelsAwayFromMagenta` passed; `swift test --filter OrbitalViewSwiftUITests` passed with 126 tests and 0 failures; `swift test` passed with 206 tests and 0 failures; the parent launcher rebuilt/re-signed/opened the review app, and `pgrep -fl "/Contents/MacOS/OrbitalViewViewer"` confirmed the visible app running from this checkout.

### Update: 2026-05-31 Ground Toggle And Geodesic Appearance Binding

- Moved the review-only ground-plane on/off control back to the left `View Detail` rail as `Ground Plane`, directly after `Speaker Numbers` and `Hidden Lines`.
- Kept the right-panel `Ground Appearance` tray focused on ground palette, grid visibility, grid spacing, and grid size.
- Fixed the Ribbed Speaker Sphere material colors so both vertical ribs and horizontal rings use the independent `Geodesic Appearance` palette and saturation instead of the neutral structure line token.
- Added regression coverage for the updated control inventory and actual SceneKit ribbed-sphere material color changes.
- Verification: `swift build` passed, `swift test` passed with 201 tests, `git diff --check` passed, and the parent launcher rebuilt/re-signed/opened the review app with `OrbitalViewViewer` relaunched from this checkout.

### Update: 2026-05-31 Fog Sphere Visual Failure Documentation

- Recorded the visible rejection of the SceneKit ribbed-sphere depth-fog shader attempt. The shader modifiers, uniforms, tests, and headless benchmark were not sufficient because the live app still did not visibly fog the sphere geometry.
- Added `docs/fog-depth-failed-attempts.md` with the full list of rejected fog attempts and the evidence standard required for the next pass.
- Restored the fog-depth item to Pending so future work does not assume the shader-preview pass was accepted.
- Verification: `git diff --check` passed. `swift build` and `swift test` were not rerun because this was a docs-only failure-analysis update after the already-built visible app was rejected.
