...
- SceneKit review-app ribbed performance regression coverage verifies default/medium/max segment count formulas, the two-node SceneKit reduction, no ribbed material/segment updates on camera-only motion, one topology rebuild when rib/ring density changes, and geodesic palette/saturation updates touching only batched rib materials
