...
- SceneKit review-app fog behavior exposes a nonlinear atmospheric depth ramp that reduces rear alpha, rear emission, speaker/source color contrast, label opacity, and Ribbed Speaker Sphere material/cutaway visibility while preserving selected speaker readability and camera-only material-skip cadence; the batched SceneKit ribbed sphere also owns camera-depth fog shader modifiers/uniforms so visible rib fragments can shade by depth inside a single batch
