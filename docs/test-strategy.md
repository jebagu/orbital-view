...
- SceneKit review-app `View Detail` stays focused on Speaker Size, Fog Density, Speaker Labels, Hidden Lines, and the display-only Ground Plane toggle
- SceneKit review-app fog behavior exposes a nonlinear atmospheric depth ramp that reduces rear alpha, rear emission, speaker/source color contrast, and label opacity while preserving selected speaker readability and camera-only material-skip cadence; current ribbed-sphere tests prove fog updates SceneKit atmosphere plus the cutaway plane without rewriting the two ribbed material batches, while the rejected shader-modifier path and later uniform full-sphere fog blanket remain excluded because neither proved accepted visible rib-depth fog
...
- SceneKit review-app `Ground Appearance` exposes Ground Palette, Grid Visibility, Grid Spacing, Grid Thickness, and Grid Size controls, while the Ground Plane on/off toggle lives in left-rail `View Detail`
...
- SceneKit review-app `Color Palette` tray uses full-width custom theme buttons, includes the Orbisonic family palette list from `orbisonic-palette-brief`, drives app skin, physical speaker pixels, source marker pixels, sphere palette, ground palette, and Cube VU ramp together, resets local sphere/ground palettes to explicit `Same as Main Theme` follow state on global changes, and replaces the removed Sonic Sphere/source speaker palette trays
...
- SceneKit review-app Cube VU controls preserve Core scalar defaults, default to 9x9 pixel density, default Cube Outline to invisible, keep outline constants delicate, apply Cube Outline to Cube VU, Pixel Jets, and Cell Jets blocks, and ignore old speaker-height values for geometry/material keys
...
- SceneKit review-app `Cube Surface` / `Jet Surface` controls expose Pixel Density capped at `1...9`, Pixel Fill, and Surface Checker Opacity, can recover the older separated-pixel face mode, can mute the forced idle checkerboard without changing geometry, and keep their dice randomizer scoped to cube/jet-surface values. Cell Jets alone exposes Idle Opacity, defaulting to 100%, and randomizers do not lower it.
- SceneKit review-app speaker type options include `Prism`, `Sphere`, `Cube VU`, `Pixel Jets`, and `Cell Jets`, with full-width tray header hit targets for collapsible tuning sections; both jet styles expose a `Jet Length` slider, persist through settings JSON, default older JSON to `48` px, and extend speaker boxes outward along the radial normal. Pixel Jets uses retained axial/cross face-pixel textures with selected VU-ramp colors, near-dark silent/no-provider materials, no SceneKit shader modifiers, and stable texture/material state when only time changes. Cell Jets uses retained five-face cell materials only, with four sleeve sides and one outward end cap per cell assigned to the same material/color, so meter updates mutate material state without rebuilding speaker geometry, unchanged meter input does not create clock-only motion, silent idle opacity can drop to fully hidden, and the final cell colors only at full-scale or clip. SceneKit and canvas speaker labels are placed from the furthest visible speaker/meter extremity plus the small Cube VU-style gap for Prism, Sphere, Cube VU, Pixel Jets, and Cell Jets.
...
- SceneKit review-app `Color Palette` drives physical speaker colors, source marker colors, app skin, ribbed-sphere colors, ground-grid colors, and Cube VU ramp together; `Sphere Palette` and `Ground Appearance` put `Same as Main Theme` first, resolve that state at render time, can locally override their palettes after an explicit local choice, and a new global `Color Palette` choice resets both local palettes to follow main without duplicating local concrete palette state
- SceneKit review-app `Sphere Geometry` exposes `Ribbed Speaker Sphere`, `Rib Thickness`, `Vertical Ribs`, and `Horizontal Rings`, defaults the ribbed overlay off, clamps Rib Thickness to `70%...250%`, derives deterministic fitted rib segments from active receiver speaker centers, uses evenly spaced symmetrical ribs/rings, and renders the SceneKit ribbed sphere through two welded curve-batched mesh nodes instead of per-segment cylinder nodes, independent open tube fragments, or ribbed-sphere shader modifiers
- SceneKit review-app `Sphere Palette` exposes only `Sphere Palette` and `Sphere Saturation`; the selected hue applies to both vertical and horizontal ribbed-sphere lanes in SceneKit and canvas fallback, and palette selection lifts default zero saturation so the hue is visible, with retained SceneKit material names, a both-lanes Rack Blue dominance regression, and a rendered snapshot pixel regression proving Rack Blue does not collapse to the rejected magenta shader fallback
...
- SceneKit review-app grid-plane visibility, spacing, thickness, and ground palette changes stay isolated from ribbed sphere update/topology keys, speaker geometry/material keys, speaker label keys, source pose/material keys, and Cube VU state
...
- SceneKit review-app global `Roll the dice on looks` icon-only action randomizes view/visual state ranges, including Color Palette, Sphere Palette, Ground Appearance, and all current `Sphere Geometry` controls, while preserving source mode, telemetry advertiser selection, local song fields, local-song render type, impulse pattern, saved/default theme metadata, selected speaker, and diagnostics
- SceneKit review-app saved theme/settings schema `9` payload includes global color palette, legacy-compatible source speaker palette field, resolved sphere palette plus follow-main flag, sphere saturation, ribbed speaker sphere visibility/thickness/rib/ring settings, speaker type, jet length, speaker label font and font size, source mode, legacy meter source/impulse pattern, audio render type, Cube VU preset/settings, performance cadence values, and the full left-panel audio/camera/view-detail state; older separate source palette fields decode safely but no longer appear as live UI controls
...
- SceneKit review-app saved theme/settings payload round-trips top-level `groundAppearance.showGridPlane`, `gridPlaneVisibilitySlider`, `gridPlaneSpacing`, `gridPlaneRenderStyle`, `gridPlanePaletteFollowsMainTheme`, and `gridPlaneThickness`, while decoding older `leftPanel.viewDetail` grid fields and defaulting missing older JSON values to `false`, `70`, default spacing, the fallback palette, inferred follow-main state, and default thickness
- SceneKit review-app grid-plane geometry keeps the canonical `z = -1.2` offset, default `0.5` spacing, default `100%` thickness, `50%...250%` thickness range, 10 x 10 bounds, deterministic default line count, alternate spacing line counts, default opacity mapping, and thickness-scaled SceneKit/canvas line widths
- SceneKit review-app camera projection maps canonical `+X = right` to screen right and `-X = left` to screen left for Plan, Elevation, and Isometric; the Isometric preset is the true normalized `+X/+Y/+Z` direction, preset camera bases stay orthonormal, spin preserves canonical Z-axis elevation, unadjusted legacy saved cameras resolve to the current preset, and DomeLab-style drag control keeps horizontal movement on yaw and vertical movement on pitch with both accepted drag directions pinned by tests
...
- SceneKit review-app ribbed performance regression coverage verifies default/medium/max segment count formulas, the two-node SceneKit reduction, open vertical rib curves, closed horizontal ring curves, shared welded tube cross-section rings, capped vertical endpoints, shader-free opaque depth-writing rib materials with modest brightness/emission lift and physically based luster, camera-only motion updating only the clipping-plane transform with no material writes or segment visits, fog changes avoiding ribbed material rewrites, one topology rebuild when rib/ring density changes, geodesic palette/saturation updates touching only batched rib materials, both vertical and horizontal lanes using the selected hue, and palette selection lifting the default zero saturation before rendered-pixel checks
...
- SceneKit review-app DEBUG probe coverage verifies the ribbed debug overlay reports the live build/style/saturation/apply/material-name state, and DEBUG-only launch flags can force ribbed visibility, suppress saved themes/random animation, trace render writes, force continuous rendering, and force the ribbed sphere green for live diagnosis
